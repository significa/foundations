...
// the nextra code highlighter doesn't support some extension variants
...
async function buildRegistry() {
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      let extension = path.extname(file.name).replace(/^\.+/, ''); // remove leading extension dot
      ...
      // remove leading '../' that added to root files by default
      if (key.startsWith('../')) {
        key = key.substring(3);
      }
    ...
    const indexContentString = [
      ...
      index.imports.map(([module, path]) => `import ${module} from '${path}';`).join('\n'),
      ``,
      `export const INDEX = {`,
      index.entries.map(([key, code]) => `'${key}': ${code}`).join(',\n'),
      `};`,
      ``,
