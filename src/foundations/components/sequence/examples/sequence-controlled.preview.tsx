...
import { Sequence, SequenceItem, SequenceItems, SequencePanel, SequencePanels } from "../sequence";
...
const SequenceControlled = () => {
  ...
  return (
    <div>
      ...
      <Sequence className="relative w-160" loop duration={3000} currentIndex={currentIndex} onChange={setCurrentIndex}>
          ...
          {CONTENT.map((item, index) => (
            <SequencePanel key={index}>
              <div className="border-border absolute top-0 left-0 flex h-full min-h-64 flex-col justify-between rounded-lg border p-4">
                <div className="text-foreground-secondary font-mono text-sm uppercase">{item.title}</div>
                <div className="pr-8 text-xl font-medium text-pretty">{item.description}</div>
              </div>
            </SequencePanel>
          ))}
        </SequencePanels>
