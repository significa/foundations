      ...
      <StepperContext value={currentIndex}>
        <div className="border-border flex flex-col gap-4 rounded-lg border p-4">
          <div className="flex gap-2">
            <Button className="grow" size="sm" onClick={() => setCurrentIndex((c) => (c - 1 + length) % length)}>
              ←
            </Button>
            <Button className="grow" size="sm" onClick={() => setCurrentIndex((c) => (c + 1) % length)}>
              →
            </Button>
          </div>
          <div className="bg-background-secondary border-border min-w-48 rounded-lg border p-8 text-center">{children}</div>
        </div>
      </StepperContext>
