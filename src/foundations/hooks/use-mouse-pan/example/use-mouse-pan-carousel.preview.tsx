...
import { Children, createContext, type ReactNode, useContext, useMemo, useState } from "react";
...
const ITEMS = [
  {
    ...
    image: "https://images.unsplash.com/photo-1505142468610-359e7d316be0?q=80&w=960&auto=format&fit=crop",
  },
  {
    title: "Forest Trail",
    description: "A winding path through an ancient forest filled with towering trees",
    image: "https://images.unsplash.com/photo-1595514807053-2c594370091a?q=80&w=960&auto=format&fit=crop",
  },
  {
    title: "Desert Dunes",
    description: "Rolling sand dunes stretching endlessly toward the horizon",
    image: "https://images.unsplash.com/photo-1498144668414-48bf526766cf?q=80&w=960&auto=format&fit=crop",
  },
  {
    title: "Mountain Sunrise",
    description: "A breathtaking view of the sun rising over misty peaks",
    image: "https://images.unsplash.com/photo-1736525155507-2326a56f0606?q=80&w=960&auto=format&fit=crop",
  },
];
      ...
      <InstanceCounterProvider>
        <div>
          ...
          >
            ...
            >
              {Children.map(children, (child) => (
                <li className="aspect-3/2 w-full snap-center overflow-hidden rounded-sm select-none">{child}</li>
              ))}
            </ul>
          </div>
          ...
          <div className="mt-3 flex w-full items-center gap-2">
            <Button disabled={activeIndex === 0} variant="ghost" size="sm" onClick={() => to(activeIndex - 1)}>
              ←
            </Button>
            <Button disabled={activeIndex === numItems - 1} variant="ghost" size="sm" onClick={() => to(activeIndex + 1)}>
              →
            </Button>
...
const CarouselItem = ({ title, description, image }: { title: string; description: string; image: string }) => {
  const index = useInstanceCounter();
  const { activeIndex, setActiveIndex } = useContext(CarouselContext);
  const isActive = activeIndex === index;

  const [ref] = useIntersectionObserver<HTMLDivElement>({ threshold: 0.9 }, (isIntersecting) => {
    if (isIntersecting) setActiveIndex(index);
  });

  return (
    <div ref={ref} className="flex size-full items-end bg-cover bg-center" style={{ backgroundImage: `url(${image})` }}>
  ...
  return (
    <Carousel>
      {ITEMS.map((item, index) => (
        <CarouselItem key={index} title={item.title} description={item.description} image={item.image} />
      ))}
    </Carousel>
  );
};
