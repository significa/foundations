...
export default function BasicHierarchicalSelection() {
  ...
  return (
    <div className="space-y-2">
      <div className="mb-4">
        <p className="text-foreground-secondary text-sm">Selected: {selectedArray.join(", ") || "None"}</p>
      </div>
          ...
          return (
            <Disclosure key={parent.id} open={isOpen}>
              <div className="flex items-center justify-between gap-2">
                ...
                <DisclosureTrigger onClick={() => toggleParentOpen(parent.id)} className="w-auto cursor-pointer rounded p-1">
                  <DisclosureChevron />
                </DisclosureTrigger>
              </div>
