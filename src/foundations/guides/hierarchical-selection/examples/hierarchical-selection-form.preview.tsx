...
function HierarchicalSelectionField({ value, onChange, data, error }: HierarchicalSelectionFieldProps) {
  const [searchQuery, setSearchQuery] = useState("");

  const { toggleItem, toggleParent, toggleParentOpen, getParentStatus, isSelected, isParentOpen } = useHierarchicalSelection({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={cn("max-h-64 space-y-1 overflow-y-auto rounded-md border p-3", error && "border-red-500")}>
    ...
    <div className="mx-auto w-full p-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-foreground-secondary mb-3 text-sm">Select the countries you are interested in for our services</p>
        ...
        <div className="bg-background-secondary rounded p-3">
          <p className="mb-1 text-sm font-medium">Selected countries ({watchedCountries.length}):</p>
