  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex gap-2">
        <Button variant="ghost" size="sm" onClick={clearSelection} disabled={selectedArray.length === 0}>
          Clear All
        </Button>

        <Button variant="ghost" size="sm" onClick={() => selectItems(["portugal", "spain", "france"])}>
          Select EU Core
        </Button>

        <Button variant="ghost" size="sm" onClick={() => selectItems(["united-states", "canada", "australia"])}>
            ...
            return (
              <Disclosure key={parent.id} open={isOpen || !!searchQuery}>
                <div className="flex items-center justify-between gap-2">
                  <label className={cn("flex flex-1 items-center gap-2", isParentDisabled ? "opacity-50" : "cursor-pointer")}>
                    <Checkbox
                    ...
                    />
                    <span className={cn("text-base font-medium", isParentDisabled ? "" : "cursor-pointer")}>
                      {parent.label}
                      {isParentDisabled && <span className="text-foreground-secondary ml-2 text-xs">(Disabled)</span>}
                    </span>
                  </label>
                  ...
                  <div className="border-border mt-2 ml-6 space-y-2 border-l-2 pl-4">
                    {parent.children?.map((child) => {
                      ...
                      return (
                        <label
                          key={child.id}
                          className={cn("flex items-center gap-2", isChildDisabled ? "opacity-50" : "cursor-pointer")}
                        >
                          <Checkbox
                          ...
                          />
                          <span className={cn("text-base font-medium", isChildDisabled ? "" : "cursor-pointer")}>
                            {child.label}
                            {child.disabled && <span className="text-foreground-secondary ml-2 text-xs">(Disabled)</span>}
                          </span>
                        </label>
                      );
                    })}
      ...
      <div className="bg-background-secondary mt-4 space-y-2 rounded p-3">
        <div>
          <p className="mb-1 text-sm font-medium">Selected countries ({selectedArray.length}):</p>
          <p className="text-foreground-secondary text-sm">{selectedArray.length > 0 ? selectedArray.join(", ") : "None"}</p>
        </div>

        <div>
          <p className="mb-1 text-sm font-medium">Opened regions ({openedArray.length}):</p>
          <p className="text-foreground-secondary text-sm">{openedArray.length > 0 ? openedArray.join(", ") : "None"}</p>
        </div>
      </div>
    </div>
  );
