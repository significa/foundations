...
export default defineConfig({
  ...
  reporter: [
    [
      'html',
      {
        outputFolder: process.env.CI ? 'playwright-report-ci' : 'playwright-report',
...
test.describe('shop the look card works', () => {
  test.beforeEach(async ({ page, context }) => {
    await page.goto('/shop-the-look', { waitUntil: 'domcontentloaded' });
    // wait for the shop the look card to load
    await page.waitForSelector('div[role="button"]', { state: 'attached' });
  });

  test('card image is the same as the preview image', async ({ page }) => {
    const firstCard = await page.getByRole('button', { name: /[0-9]+ item(s)/ }).first();
    ...
    const imageSrc = (rawImageSrc as string).replace(/\/m\/\d+x\d+\//, '/') || '';
    ...
    const drawerImage = await drawer
      .getByRole('img')
      .filter({
    ...
    const previewImageSrc = (rawPreviewImageSrc as string).replace(/\/m\/\d+x\d+\//, '/') || '';
  ...
  test('consistent number of products', async ({ page }) => {
    ...
    const markerButtons = await drawer.getByRole('button', { name: /^marker-/ });
    await expect(markerButtons).toHaveCount(1);
    const markerButtonsCount = await markerButtons.count();

    const wishListButton = drawer.getByRole('button', { name: 'Add to wishlist' }).first();
    ...
    const productCards = await slider.locator(':scope > *');
...
9. Upload HTML report to the GitHub UI
