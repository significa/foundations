...
But enough of the big words, well, what we are trying to say is:
...
Playwright will download all the necessary browsers and create the following files.
...
- Default timeout is 30s for each test case. This can be increased as you see fit, but be mindful of the time so that the test execution does not take too long
- Reports generated locally should be separated from the CI reports
- webServer is set up so that before the Playwright CI runs the test script, the page is served; this should be modified to suit the project
...
Aside from using `console.log` and `debugger`, Playwright UI mode offers a better debugging experience, where you can easily walk through each step of the test and visually see what was happening before, during and after each step.
...
You can also debug in live mode with the VSCode extension or right-click and select debug test (Just install the VSCode extension for a better experience).
...
- Focus on user-visible behaviour (i.e. test things that real users interact with, like colour, texts, UI elements, etc)
...
- Centralise configuration in `playwright.config.ts` to maintain consistency across test runs.

Pheew! That's a lot of dos and don'ts, but depending on your use case, these guidelines can be adjusted to fit the needs of your tests. After all, Playwright gives you the flexibility to make testing work your way! 🚀.

## Test Reports

Playwright provides built-in reporting capabilities.  
Run `npx playwright show-report` to view the report. This will generate a report by default as an HTML page in the playwright-report folder.
...
1. Run on a schedule (in the example above, it runs on every Wednesday 00:00 UTC)
