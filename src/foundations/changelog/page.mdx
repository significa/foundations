...
### New: `<Field>` component (and removal of standalone `<Label>`)
...
### Bug fix: Tabs indicator animating between pages
...
### Page dependency validation in CI

A new `scripts/check-page-deps.mjs` script runs as part of `pnpm check` (via `pnpm check:deps`). It walks every component's source files and fails CI if any cross-Foundations import (`@/foundations/...`) isn't declared in the corresponding `page.mdx` `dependencies:` block.

This caught and corrected several existing omissions: Tooltip and Input were missing `Slot`, Slider was missing `Slot` + `composeRefs`, Color Picker was missing `composeRefs`, Toaster was missing `useTopLayer`, and Listbox had a typo (`/foundations/ui/input` → `/ui/input`). The dependency lists each component's `page.mdx` advertises now match what a consumer actually drags in when they copy the source.

Imports from `@/lib/...` (utilities provided by the setup guide) are skipped automatically. External `dependencies` entries with `https://` hrefs aren't validated. The check normalises trailing slashes and `#anchor` suffixes, so `/utils/math#clamp` and `/utils/math` both match a `utils/math` import.
...
### Bug fix: Tabs panels all rendering on first paint
...
### Reduced motion support in Disclosure, Tabs and Toaster
