...
### Changed: `Input` handles `file` and `time` natively

`<Input type="file">` used to render the browser's chunky native `::file-selector-button` inside the input frame — a control-inside-a-control, with vertical alignment that disagreed across Chrome and Firefox. The selector button is now restyled to read as a flex-centered label inside the input. `<Input type="time">` used to render Chromium's clock icon and popover, which competed visually with primitives like `DatePicker`; the indicator is now suppressed in Chromium, matching Firefox and Safari which already render time inputs cleanly. Date / datetime-local / month / week aren't touched — Firefox doesn't expose a pseudo-element to hide their calendar button, so we let the native chrome through everywhere for consistency. Reach for `DatePicker` when you want a unified picker UI across browsers.
