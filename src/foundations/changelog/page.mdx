...
### Fixed: `Switch` looked square and distorted on press at small `--radius`

`Switch` used `rounded-xl` on both its track and thumb. That class derives from the global
`--radius` dial, so once `--radius` was lowered the switch rendered as a rounded square. The
press feedback made it worse: an anisotropic `scale-x-110` combined with a hardcoded
`rounded-[--spacing(2.25)]` sheared the near-square thumb into an oval blob.

A switch is a structurally round control, like `Slider`, `Radio`, and `Spinner` — its shape
carries the toggle metaphor and should not follow the surface-radius dial. Both the track and
thumb are now `rounded-full`, which also lets the directional press-stretch drop its radius
override: a capsule scaled on one axis stays a capsule. Update your copy:
...
+   "relative h-6 w-11 cursor-pointer rounded-full bg-foreground/20 transition",
    // circle
-   "before:absolute before:top-0.5 before:left-0.5 before:size-5 before:rounded-xl ...",
...
### Changed: `Table` no longer wraps itself in a scroll `div`; use `Table.Container`

`Table` used to render an implicit `<div className="relative w-full overflow-x-auto">` around
the `<table>`. Per the CSS Overflow spec, `overflow-x: auto` with `overflow-y: visible` forces
`overflow-y` to compute to `auto`, so that hidden wrapper was itself a scroll container on both
axes — but with no height constraint it never scrolled, yet it still claimed the sticky
header's scroll context. A consumer's own `max-h-*` wrapper therefore couldn't drive a sticky
header; it scrolled the whole block, header included. This was broken in every browser.

`Table` now renders only the `<table>`. The scroll container is an explicit `Table.Container`
part (a `div` with `relative w-full overflow-x-auto`, plus a `bordered` prop — default `true` —
for the rounded card). Wrap your tables in it where you relied on horizontal overflow or the
card border, and set any `max-h-*` for a sticky header on the container itself:
...
A plain `<Table>` with no container is still valid for a table that flows inline with the page.

### Changed: `Switch` now exposes `role="switch"`

`Switch` is a native `<input type="checkbox">`, so it announced as a checkbox to assistive
technology even though it's a binary on/off toggle. Adding `role="switch"` gives it the
accurate role; `aria-checked` is still derived from the input's `checked` state, so no manual
state management is needed and form submission/keyboard behavior is unchanged.
...
### Fixed: form controls rendered `invalid` styling permanently

`Input`, `Textarea`, `Select`, and `Listbox` spread `data-invalid` / `aria-invalid`
unconditionally. React renders a `false` value on these attributes as the literal string
`"false"`, and the invalid styles key off attribute _presence_ (`[data-invalid]`,
`:has([data-invalid])`) — so an `<Input invalid={false} />` still matched the selector and
showed error styling, plus emitted a noisy `aria-invalid="false"`. Render the attributes only
when `invalid` is truthy:
...
// listbox.tsx — ListboxTrigger
- data-invalid={ctx.invalid}
+ data-invalid={ctx.invalid || undefined}
```

### Fixed: triggers could submit their enclosing form

A bare `<button>` defaults to `type="submit"`, so several primitives submitted the
surrounding `<form>` when clicked. `Modal.Trigger`, `Modal.Close`, and `Listbox.Option`
were missing `type="button"` outright — apply this fix to your copies:

```diff tsx
// modal.tsx — ModalTrigger and ModalClose
- <Component {...props} onClick={handleClick}>
+ <Component type={asChild ? undefined : "button"} {...props} onClick={handleClick}>

// listbox.tsx — ListboxOption
  <button
    ref={ref}
+   type="button"
    role="option"
...
The `Slot` primitive now also defaults `type="button"` for `asChild` triggers — but only
when the rendered child is a native `<button>` with no explicit `type`. Non-button children
(`<a>`, `<div role="button">`, custom components) and buttons with an explicit
`type="submit"` are left untouched. This makes the `type={asChild ? undefined : "button"}`
pattern form-safe even in `asChild` mode, so triggers (Popover, Tooltip, Tabs, Toggle,
SegmentedControl, Disclosure, Button…) no longer rely on each call site getting `type` right.
Update your `slot.tsx`:
...
If you previously relied on an `asChild` trigger submitting a form via a bare `<button>`
child, set `type="submit"` on that child explicitly.

### Removed: `Textarea.Resize`

`Textarea.Resize` carried a JS auto-grow implementation (a `useEffect` measuring
`scrollHeight`). Now that `field-sizing: content` is supported across all major
engines (Chrome/Edge 123, Firefox 152, Safari/iOS 26.2), it's gone — auto-resize
is a single class on the base `Textarea`. Add `field-sizing-content`, and pair it
with a `max-h-*` (and `overflow-auto`) class to bound the growth, since
`field-sizing: content` grows without a ceiling on its own. Browsers without
support fall back to a fixed-height textarea. Update call sites:

```diff tsx
- <Textarea.Resize />
+ <Textarea className="field-sizing-content max-h-48 overflow-auto" />
```

The `composeRefs` dependency is no longer needed.

### Fixed: `Drawer.Content` rendered with a transparent panel

`Drawer.Content` set its surface with `bg-background-high` — a token that doesn't
exist in the setup theme. Tailwind emits no rule for an undefined token, so the
panel had no background and the blurred backdrop showed straight through it. The
surface now uses `bg-background`, matching every other overlay primitive (Dialog,
Popover, Menu, Listbox). Apply this to your copy:
...
### Added: `Drawer` placement via `side`

`Drawer.Content` now accepts a `side` prop:
`"left" | "right" | "bottom" | "left-bottom" | "right-bottom"` (default
`"right-bottom"`, which matches the previous hard-coded behaviour, so no change is
required). Single-edge values pin to that edge at every breakpoint; hyphenated
values are `{desktop}-{mobile}` — a desktop-edge panel that becomes a bottom sheet
on mobile. Placement is implemented as a `cva` `side` variant; copy the new
`drawerContentStyle` object into your `drawer.tsx`.

### Changed: `Drawer` detachment and corners follow `--radius`

The drawer's roundness and its detachment from the window edge now derive from the
global `--radius` dial (the same system `--inset` uses for panel items), instead of
the hard-coded `rounded-t-xl` / flush edges. At `--radius: 0` the drawer is flush
and square as before; as roundness increases it floats off the edge with matching
rounded corners. The detachment magnitude is `calc(var(--radius) * 3 * var(--radius-bump))` —
tune the multiplier to taste.

### Fixed: nested modals no longer all close on Escape

`Modal` intercepted Escape with an element `keydown` listener (plus a `window`
fallback). When modals are nested in the DOM, that keydown bubbled through every
ancestor dialog, so one Escape closed the whole stack. Fix: add
`event.stopPropagation()` to the element `keydown` handler so it can't reach
ancestor dialogs, and drop the `window` listener. The `keydown` `preventDefault()`
must stay — Escape can otherwise push past a prevented `cancel` and close the dialog
natively, dropping it from the top layer mid-transition.
...
### Added: stacked drawers recede instead of compounding

Drawers stack via the native `<dialog>` top-layer, but each painted its own
backdrop so the dimming compounded. Now only the **outermost** backdrop shows: any
drawer nested inside another open drawer hides its own backdrop
(`in-data-[status=open]:backdrop:opacity-0`), and the single bottom layer dims the
page for the whole stack — so the dim never flashes when you open or close a level.
Covered drawers also shift inward via `margin` (`has-data-[status=open]:`) so the
stack peeks out. It's pure CSS — no JavaScript, no stack bookkeeping — so it applies
to drawers nested in the DOM. Margin, not transform: a transform on a drawer that
DOM-contains another dialog would become that dialog's containing block and break
its positioning. Relatedly, `Modal.Content` now composes a forwarded `ref` (it
previously let a spread `ref` clobber its internal refs).
...
### Changed: `Input` handles `file` and date/time natively

`<Input type="file">` used to render the browser's chunky native `::file-selector-button` — a control-inside-a-control, with vertical alignment that disagreed across Chrome and Firefox. The selector button is now flex-centered inside the input frame and renders consistently across browsers. `<Input type="time">`, `type="date"`, `type="datetime-local"`, `type="month"`, and `type="week"` used to render Chromium's calendar/clock picker icon plus a native popover that competed visually with primitives like `DatePicker`; that indicator is now suppressed in Chromium (Safari already rendered without chrome). Firefox keeps its native calendar button on the date family because it doesn't expose a pseudo-element to hide it — reach for `DatePicker` when you want a unified picker UI across browsers.

If your copy of `input.tsx` needs the same fix, splice these classes onto the input element (both standalone and inside `Input.Group`):
...
[Avatar](/ui/avatar) now ships an `Avatar.Group` sub-component for stacked avatars. Each non-leading avatar is cut by an SVG mask shaped after the previous avatar's variant and size, so the gap is transparent rather than a painted ring. Direct `<Avatar>` children only — others are filtered out.
