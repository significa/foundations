...
### Changed: `Input` tames native chrome for `file` and date/time types

`<Input type="file">` used to render the browser's chunky native `::file-selector-button` inside the input frame — a control-inside-a-control. `<Input type="time">` (and its `date` / `datetime-local` / `month` / `week` siblings) rendered Chromium's calendar/clock icon plus a native popover, which competed visually with primitives like `DatePicker`. Both are now neutralized at the base: the file selector button reads as inline text inside the frame, and the calendar-picker indicator is hidden. Keyboard entry still works; the native popover is no longer reachable by mouse. For a visual cue, drop a `CalendarIcon` / `ClockIcon` into an `Input.Addon`. For a full picker UI, use `DatePicker`.
...
To opt back into the native picker per-instance:

```tsx
<Input
  type="time"
  className="[&::-webkit-calendar-picker-indicator]:block"
/>
```
