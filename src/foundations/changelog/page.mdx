...
A new [`SegmentedControl`](/ui/segmented-control) for switching between a small set of mutually-exclusive options. Animated indicator via `motion`'s `layoutId`, full WAI-ARIA radiogroup wiring, arrow-key navigation, and controlled/uncontrolled state.
...
Items accept `asChild` for polymorphic rendering.
