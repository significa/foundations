...
### Added: Charts and Blocks

Two new categories. **Charts** ([Area](/charts/area-chart), [Bar](/charts/bar-chart),
[Line](/charts/line-chart)) are hand-built on plain SVG via a shared
[Chart Container](/charts/chart-container) — **zero runtime dependencies**, no charting
library and no d3. **Blocks** ([Auth Form](/blocks/auth-form),
[Sidebar Layout](/blocks/sidebar-layout)) are full sections assembled from existing
primitives, meant to be copied whole — browse them in the [Blocks gallery](/blocks).

Charts read their colours from a new five-token OKLCH palette. If you want to use them,
add it to your theme's `@theme` block (it sits alongside the existing semantic colours
and swaps automatically in dark mode):

```diff css
  --color-info: oklch(62.3% 0.214 259.815);
+
+ --color-chart-1: light-dark(oklch(62.3% 0.214 259.815), oklch(70% 0.18 259.815));
+ --color-chart-2: light-dark(oklch(69.6% 0.17 162.48), oklch(74% 0.15 162.48));
+ --color-chart-3: light-dark(oklch(79.5% 0.184 86.047), oklch(82% 0.17 86.047));
+ --color-chart-4: light-dark(oklch(63.7% 0.237 25.331), oklch(70% 0.2 25.331));
+ --color-chart-5: light-dark(oklch(60% 0.22 300), oklch(70% 0.19 300));
```
