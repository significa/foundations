 ...
 * @returns
 */
const PopoverPanel = ({ ref, context, modal, className, style, ...props }: PopoverPanelProps) => {
...
const PopoverSearchInput = ({ className, ...props }: React.ComponentPropsWithRef<"input">) => {
  return (
    <div className="border-border relative flex items-center rounded-t-lg border-b bg-transparent">
      <MagnifyingGlassIcon weight="bold" className="text-foreground absolute left-4 size-4 shrink-0" />
      <input
        className={cn(
