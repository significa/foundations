...
<PropsTable
  definition={{
    ...
    origin: {
      ...
      description:
        'How the popover is anchored. `"trigger"` (default) anchors to the trigger element. `"pointer"` captures the click coordinates on the trigger and anchors there. A `[clientX, clientY]` tuple anchors to that explicit point — useful for right-click context menus.',
      default: '"trigger"',
    },
    modal: {
      type: "boolean",
...
<PropsTable
  definition={{
    isLoading: {
      type: "boolean",
      description: "When true, replaces the leading magnifying-glass icon with a spinner. Useful for async-filtered content.",
      default: "false",
    },
  }}
/>
