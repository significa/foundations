...
interface SliderProps extends Omit<React.ComponentPropsWithRef<"div">, "onChange" | "defaultValue"> {
  min?: number;
  max?: number;
  step?: number;
  value?: number;
...
const Slider = ({
  ...
  const contextValue = useMemo(
    ...
    [value, setValue, min, max, step, orientation, disabled, onValueChange, progressFactor]
  );
...
const sliderTrackStyle = cva({
  base: ["bg-foreground-secondary/40 pointer-events-none absolute rounded-full"],
  variants: {
    orientation: {
      horizontal: "left-[calc(var(--thumb-size)/2)] h-(--track-thickness) w-[calc(100%-var(--thumb-size))]",
      vertical: "bottom-[calc(var(--thumb-size)/2)] h-[calc(100%-var(--thumb-size))] w-(--track-thickness)",
    },
  },
  defaultVariants: {
    orientation: "horizontal",
...
const SliderTrack = ({ className, asChild, ref, ...props }: SliderTrackProps) => {
  const { orientation } = useSliderContext();
  const Comp = asChild ? Slot : "div";

  return <Comp ref={ref} className={cn(sliderTrackStyle({ orientation }), className)} tabIndex={-1} {...props} />;
};

const sliderRangeStyle = cva({
  base: ["bg-accent/90 pointer-events-none absolute z-5 rounded-full"],
  variants: {
    orientation: {
...
const SliderRange = ({ className, asChild, ref, style, ...props }: SliderRangeProps) => {
  const { orientation } = useSliderContext();
  const Comp = asChild ? Slot : "div";

  const rangeStyle = {
    [orientation === "horizontal" ? "width" : "height"]: `calc(var(--progress-track-factor) * 100%)`,
    ...style,
  };

  return (
    <Comp ref={ref} className={cn(sliderRangeStyle({ orientation }), className)} style={rangeStyle} tabIndex={-1} {...props} />
  );
};
...
const sliderThumbStyle = cva({
  base: [
    ...
    "[&::-webkit-slider-thumb]:bg-foreground [&::-webkit-slider-thumb]:h-(--thumb-size) [&::-webkit-slider-thumb]:w-(--thumb-size)",
    "[&::-webkit-slider-thumb]:z-10 [&::-webkit-slider-thumb]:rounded-full",
    "[&::-moz-range-thumb]:transition",
    "[&::-moz-range-thumb]:bg-foreground [&::-moz-range-thumb]:h-(--thumb-size) [&::-moz-range-thumb]:w-(--thumb-size)",
...
type SliderThumbProps = Omit<React.ComponentPropsWithRef<"input">, "value" | "type">;

const SliderThumb = ({ className, onChange, ref, ...props }: SliderThumbProps) => {
  const { value, setValue, min, max, step, orientation, disabled } = useSliderContext();
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const sliderElement = e.target.closest('[style*="--progress-track-factor"]') as HTMLElement;
      if (sliderElement) {
        sliderElement.style.setProperty("--progress-track-factor", `${newValue / max}`);
      }
