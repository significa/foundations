...
export default function DropdownSearchCreatePreview() {
  ...
  const onOpenChange = (open: boolean) => {
    if (!open) {
      setTimeout(() => {
        setSearch("");
        setPeople((prev) => [...selected, ...prev.filter((person) => !selected.some((s) => s.id === person.id))]);
  ...
  return (
    <div className="border-border w-96 rounded-3xl border p-4">
      <Dropdown onOpenChange={onOpenChange}>
        ...
        <DropdownItems>
          <DropdownSearchInput placeholder="Search people" value={search} onChange={(e) => setSearch(e.target.value)} />
          {filteredPeople.map((person) => (
            ...
            >
              <Checkbox className="pointer-events-none" checked={selected.some((p) => p.id === person.id)} readOnly />
              <span>{person.name}</span>
            </DropdownItem>
          ))}
          {filteredPeople.length === 0 && <DropdownEmpty>No results</DropdownEmpty>}
          {search && (
            <>
              <DropdownDivider />
              <DropdownItem
