  ...
  return (
    <div className="w-80">
      <Listbox value={selectedPerson} onChange={setSelectedPerson}>
        <Listbox.Trigger placeholder="Select person" className={selectedPerson ? "pl-3" : ""}>
          {selectedPerson && (
        ...
        <Listbox.Options>
          {people.map((person) => (
            <Listbox.Option key={person.id} value={person} className="flex items-center gap-2 px-3">
              <Avatar size="xs">
                <Avatar.Image src={`https://api.dicebear.com/6.x/thumbs/svg?seed=${person.name}`} />
