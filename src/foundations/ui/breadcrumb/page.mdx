...
Extends the `span` element. Decorative collapse indicator. Combine with a `Dropdown` (or any disclosure) to expand the hidden segments.
...
When the trail is too long, replace middle segments with `Breadcrumb.Ellipsis` inside a `Dropdown` so they remain reachable.
