...
const badgeVariants = cva({
  base: "inline-flex items-center gap-1 rounded-full leading-none font-semibold ring-1 ring-inset [&>[data-badge-icon]:first-child]:-ml-0.5 [&>[data-badge-icon]:last-child]:-mr-0.5",
  variants: {
    variant: {
...
const Badge = ({ ref, children, variant = "neutral", size = "md", className, asChild, ...rest }: BadgeProps) => {
  ...
  return (
    <Comp ref={ref} className={cn(badgeVariants({ variant, size }), className)} {...rest}>
      {children}
    </Comp>
  );
};
