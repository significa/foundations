...
<PropsTable
  definition={{
    ...
    onOpenChange: {
      ...
      description: "Callback function called when the tooltip's open state changes.",
    },
    initialOpen: {
      type: "boolean",
      description: "Whether the tooltip is open by default. Useful when used uncontrolled.",
      default: false,
    },
    placement: {
      type: "Placement",
...
<PropsTable
  definition={{
    asChild: {
      type: "boolean",
      description: "When true, the trigger will merge props with its child element instead of rendering as a button.",
      default: false,
    },
  }}
/>
...
Tooltips placed within a `Tooltip.Group` won't have delay between them, creating a smooth experience when moving between multiple tooltips.
