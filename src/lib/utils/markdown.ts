...
const link = (text: string, href: string) => {
  ...
  const transformedHref = isInternal ? `${new URL(SITE_ORIGIN).origin}${href}` : href;
...
const createComponentResolver = (components: ComponentDefinition<z.ZodType>[]) => {
  return async (mdxContent: string) => {
    ...
    for (const node of tree.children) {
      ...
      if (componentDef) {
        ...
        if (!node.position) {
          throw new Error(`Node position is missing for component ${componentDef.name}`);
        }

        const rawComponent = mdxContent.slice(node.position.start.offset, node.position.end.offset);
